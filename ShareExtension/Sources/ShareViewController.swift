...
class ShareViewController: SLComposeServiceViewController {
    ...
    private func upsertLink(input: ExtensionInput) {
        ...
        let existing: LinkEntity? = {
            let byOriginal = FetchDescriptor<LinkEntity>(predicate: #Predicate { $0.originalUrl == originalURL })
            if let found = try? context.fetch(byOriginal), let first = found.first { return first }
            let byCanonical = FetchDescriptor<LinkEntity>(predicate: #Predicate { $0.canonicalUrl == canonicalURL })
            if let found = try? context.fetch(byCanonical), let first = found.first { return first }
            return nil
        }()
    ...
    private func finishSuccessfully(authAware: Bool) {
        if authAware {
            // Close immediately; Post button already tapped.
            extensionContext?.completeRequest(returningItems: nil)
        } else {
            // Offer to open app (no token yet). We dismiss quickly after attempt.
    ...
    private func cancelRequest(with message: String) {
        let err = NSError(domain: "HamrahShare", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
...
extension ShareViewController {
    private func hasAuthToken() -> Bool {
        ...
        let accessGroup = (Bundle.main.object(forInfoDictionaryKey: "AppIdentifierPrefix") as? String).map { "\($0)app.hamrah.ios" }
    ...
    struct ExtensionInput { let url: URL; let title: String?; let sharedText: String?; let sourceApp: String? }

    fileprivate func extractInput(from context: NSExtensionContext, completion: @escaping (Result<ExtensionInput, Error>) -> Void) {
        guard let item = context.inputItems.first as? NSExtensionItem, let attachments = item.attachments, !attachments.isEmpty else {
            return completion(.failure(NSError(domain: "Hamrah", code: 0, userInfo: [NSLocalizedDescriptionKey: "No input item"])))}
        var foundURL: URL?
        ...
        for provider in attachments {
            ...
            if provider.hasItemConformingToTypeIdentifier("com.apple.linkpresentation.metadata") {
                group.enter()
                provider.loadItem(forTypeIdentifier: "com.apple.linkpresentation.metadata", options: nil) { (item, _) in
                    defer { group.leave() }
                    if let meta = item as? LPLinkMetadata, let title = meta.title { foundTitle = title }
                }
            }
        }
        group.notify(queue: .main) {
            guard let url = foundURL else {
                return completion(.failure(NSError(domain: "Hamrah", code: 0, userInfo: [NSLocalizedDescriptionKey: "No URL found"])))}
            completion(.success(ExtensionInput(url: url, title: foundTitle, sharedText: foundText, sourceApp: foundSourceApp)))
        }
    }
}
#endif
